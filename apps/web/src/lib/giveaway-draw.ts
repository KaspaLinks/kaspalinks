...
export async function finalizeGiveaway(publicId: string, ipHash: string) {
  ...
  if (candidate.drawProtocolVersion >= 2) {
    return drawVerifiableGiveaway(publicId, ipHash);
  }
  return drawLegacyGiveaway(publicId, ipHash);
}
