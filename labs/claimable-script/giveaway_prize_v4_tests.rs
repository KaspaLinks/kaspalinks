...
//
// Usage: copy this file plus `giveaway_prize_v4.sil` to
// `silverscript-lang/tests/` inside the pinned kaspanet/silverscript
// checkout (include giveaway_prize_v4_prototype_vectors.json), then:
//
...
const SOURCE: &str = include_str!("giveaway_prize_v4.sil");
...
fn compile_v3(init_state: [u8; 32]) -> silverscript_abi::SilAbiArtifact {
    ...
    .expect("giveaway_prize_v4.sil compiles")
}
...
fn run_web_prototype_vector(index: usize, tamper: bool) -> Result<(), TxScriptError> {
    ...
    let fixture: serde_json::Value = serde_json::from_str(include_str!("giveaway_prize_v4_prototype_vectors.json")).unwrap();
...
#[test]
fn empty_freeze_and_early_refund_pass() {run_web_prototype_vector(4,false).unwrap();run_web_prototype_vector(5,false).unwrap();}
#[test]
fn early_refund_rejects_open_nonempty_and_before_close() {for i in 6..9 {assert!(run_web_prototype_vector(i,false).is_err(), "invalid early refund {}",i);}}
#[test]
fn one_participant_draw_passes() {run_web_prototype_vector(9,false).unwrap();}
#[test]
fn empty_refund_rejects_output_tampering() {assert!(run_web_prototype_vector(5,true).is_err());}
