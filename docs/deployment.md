...
- `GIVEAWAY_LAB_ENABLED` defaults to `false` and separately gates the address-giveaway experiment. It stores participant payout addresses and public draw terms, but no wallet keys or funds. The default flow parks the exact prize in a visible one-time claimable output before entries open. Draw protocol v2 freezes sorted entry hashes into a Merkle root, fixes a future Kaspa virtual-chain blue-score target, waits for the selected chain block to be confirmed, and derives the winner from the committed seed plus that block hash. Public clients can recompute the root, freeze commitment, digest, and winner locally. After the draw, the creator browser signs an immutable payout to the selected winner address; the winner can trigger that already signed payout during the configured claim window. If it is not claimed in time, only the creator's browser-held refund key can recover the prize after the on-chain refund lock. Claim/refund keys remain encrypted in the creator's browser and private recovery file, and the server receives only signed transaction JSON that cannot redirect the payout.
